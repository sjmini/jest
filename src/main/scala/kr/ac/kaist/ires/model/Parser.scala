...
object Parser extends ESParsers {
  ...
  lazy val IfStatement: ESParser[IfStatement] = memo(args => {
    ...
    log((
      log((if (!pElse) (((MATCH <~ t("if")) <~ t("(")) ~ Expression(List(true, pYield, pAwait)) <~ t(")")) ~ Statement(List(pYield, pAwait, pReturn, false)) ^^ { case _ ~ x0 ~ x1 => IfStatement0(x0, x1, args) } else MISMATCH))("IfStatement0") |
      log(((((MATCH <~ t("if")) <~ t("(")) ~ Expression(List(true, pYield, pAwait)) <~ t(")")) ~ Statement(List(pYield, pAwait, pReturn, true)) <~ t("else")) ~ Statement(List(pYield, pAwait, pReturn, pElse)) ^^ { case _ ~ x0 ~ x1 ~ x2 => IfStatement1(x0, x1, x2, args) })("IfStatement1")
    ))("IfStatement")
  })
